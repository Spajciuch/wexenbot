...
module.exports.run = async(client, message, args, config) => {
  ...
  await firebase.database().ref(`/ustawienia/${message.guild.id}/admin`).once("value")
    .then(async admin => {
      ...
      if (!message.member.hasPermission("BAN_MEMBERS", false, true, true)) { return eval(config.no_permissions); }

      const member = message.mentions.members.first();
      if (!member) { return message.reply("<a:banhammer:460519287191240714> You have to mention someone."); }
      if (!member.bannable) { return message.reply("<a:banhammer:460519287191240714> I can't do that! Do I have enough permissions?"); }

      let reason = args.slice(1).join(" ");
      if (!reason) { reason = "Not specified reason"; }
